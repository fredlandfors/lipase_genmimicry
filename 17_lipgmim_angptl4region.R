# Endothelial lipase (EL, LIPG) activity vs. ANGPTL4 region plot
library(data.table)
library(ggplot2)

# Get lipg mimicry data ----
ssi.lipg.angptl4 <- data.table::fread(file = "./data/ssi_lipg_angptl4.txt")
ssi.lipg.angptl4 <- ssi.lipg.angptl4[rsid != ".", ]

# Get ld matrix ----
# ldm <- ieugwasr::ld_matrix(ssi_angptl4$rsid, with_alleles = FALSE, pop = "EUR")
#data.table::fwrite(as.data.frame(ldm), file = "/Users/fredriklandfors/projekt/lipase_genmimicry/data/lipg/angptl4_ldm.txt", sep = "\t")
...
p1 <- ggplot(data = plot_df, aes(x = pos.ssi, y = r2.ssi)) + 
  ggtitle("Endothelial lipase (EL, LIPG) activity R\U00B2 of variants in the ANGPTL4 region (19:8,229,011-8,629,011)") + 
  ylab("Explained LIPG activity [rs77960347-G] (R\U00B2 %)") +
  xlab("Chromosomal position (base pairs)") +
  geom_point(aes(color = ld.rs116843064, shape = E40K)) +
  geom_text(aes(label = rs116843064), vjust = 0.25, hjust = -0.1,
...
# Write to file
ggsave(plot = p1,
       path = "/Users/fredriklandfors/projekt/lipase_genmimicry/plots/sensitivity/lipg_angptl4/",
       filename = "lipg_angptl4.pdf",
...
# Write to file
pdf(
  file = "/Users/fredriklandfors/projekt/lipase_genmimicry/plots/sensitivity/lipg_angptl4/lipga4_organismdb_track.pdf",
  width = 5*1.5 * 0.7945, 
  height = 2
) 
p.txdb + 
