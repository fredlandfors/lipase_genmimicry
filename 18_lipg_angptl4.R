...
## Get data ----
cbind_df <- data.table::fread(file = paste0(wd, "/data/4_lipg_Nightingale20.txt"))
cbind_df <- as.data.frame(cbind_df)

# Remove scaling param
cbind_df <- subset(cbind_df, gwas_id.LPL != "met-d-Total_C")

## Multiple comparisons correction ----
m_comp = 9

## LIPG rs77960347 [N396S] decreases LIPG activity, harmonize betas
...
### LIPG vs. ANGPTL8 ----
EL_angptl4_plot <- scatter_plot(
  cbind_df,
  title = "HUMAN PLASMA: EL (LIPG) vs. ANGPTL4",
  subtitle = "Systemic effects on metabolic parameters",
  y_var = "beta.kol.LIPG",
  x_var = "beta.kol.ANGPTL4",
  scale_var = "kol.",
  y_gene = "LIPG",
  x_gene = "ANGPTL4",
...
### Save ----
ggsave(plot = EL_angptl4_plot,
       path = paste0(wd, "/plots/"),
       filename = "18_lipg_angptl4_Nightingale20.pdf",
       width = scatter.width, height = scatter.height, units = "in",
       dpi = 1100)

## Regression stats ----
f1 <- lm(formula = beta.kol.LIPG ~ beta.kol.ANGPTL4, data = cbind_df)
s1 <- summary(f1)
...
ggsave(plot = EL_angptl4_plot,
       path = paste0(wd, "/plots/"),
       filename = "18_lipg_angptl4_Kettunen16.pdf",
       width = scatter.width, height = scatter.height, units = "in",
       dpi = 1100)
